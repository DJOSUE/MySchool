        	        ...
        	        <?php $active = $n++;?>
	 		            <div class="tab-pane <?php if($active == 1) echo 'active';?>" id="<?php echo $row2['username'];?>">
				            <div class="row">	
		  						            ...
		  						            <div class="pi-body bglogo">
									            ...
									            <div class="pi-info">
			  							            <div class="h6 pi-name">
											            ...
											            <small><?php echo getPhrase('roll');?>: <?php echo $this->db->get_where('enroll' , array('student_id' => $row2['student_id']))->row()->roll;?></small>
			  							            </div>
			  							            <div class="pi-sub">
											            <?php echo getPhrase('class');?>: <?php echo $this->crud->get_class_name($row['class_id']); ?><br>
        									            <?php echo getPhrase('section');?>: <?php echo $this->db->get_where('section' , array('section_id' => $row['section_id']))->row()->name; ?>
			  							            </div>
									            </div>
		  						            </div>
						                </div>
				                ...
				                <?php 
    				                $student_info = $this->db->get_where('enroll' , array('student_id' => $row2['student_id'] , 'year' => $running_year))->result_array();
    				                $exams = $this->db->get_where('units')->result_array();
    					                ...
    					                <div class="table-responsive">
      						                <table class="table table-lightborder">
                    							...
                    							<?php 
                                                	...
                                                	'subject_id' => $row3['subject_id'], 'unit_id' => $row2['unit_id'],'class_id' => $row1['class_id'], 'student_id' => $row1['student_id'],'year' => $running_year));
                                        		...
                                        		?>
                          							<tr>
                										...
                										<td><?php $mark = $this->db->get_where('mark' , array('subject_id' => $row3['subject_id'], 'unit_id' => $row2['unit_id'], 'student_id' => $row1['student_id'], 'year' => $running_year))->row()->labtotal;?>
                                                            <?php if($mark < $min || $mark == 0):?>
                                                            <a class="btn btn-rounded btn-sm btn-danger" style="color:white"><?php if($this->db->get_where('mark' , array('subject_id' => $row3['subject_id'], 'unit_id' => $row2['unit_id'], 'student_id' => $row1['student_id'], 'year' => $running_year))->row()->labtotal == 0) echo '0'; else echo $mark;?></a>
                                                            <?php endif;?>
                                                            <?php if($mark >= $min):?>
                                                            <a class="btn btn-rounded btn-sm btn-info" style="color:white"><?php echo $this->db->get_where('mark' , array('subject_id' => $row3['subject_id'], 'unit_id' => $row2['unit_id'], 'student_id' => $row1['student_id'], 'year' => $running_year))->row()->labtotal;?></a>
                                                            <?php endif;?>
                                                        </td>
                                                        <td><?php echo $grade = $this->crud->get_grade($this->db->get_where('mark' , array('subject_id' => $row3['subject_id'], 'unit_id' => $row2['unit_id'], 'student_id' => $row1['student_id'], 'year' => $running_year))->row()->labtotal);?></td>
                                                      	<td><?php echo $this->db->get_where('mark' , array('subject_id' => $row3['subject_id'], 'unit_id' => $row2['unit_id'], 'student_id' => $row1['student_id'], 'year' => $running_year))->row()->comment; ?></td>
                                                        ...
                                                        <td><a class="btn btn-rounded btn-sm btn-primary" style="color:white" href="<?php echo base_url();?>parents/subject_marks/<?php echo $data;?>/<?php echo $row2['unit_id'];?>/"><?php echo getPhrase('view_all');?></a></td>
                          							</tr>
          							                <?php endforeach;} endforeach;?>
        						                </tbody>	
      						                </table>
      						                <div class="form-buttons-w text-right">
                                                <a target="_blank" href="<?php echo base_url();?>parents/marks_print_view/<?php echo base64_encode($row1['student_id'].'-'. $row2['unit_id']);?>/"><button class="btn btn-rounded btn-success" type="submit"><i class="picons-thin-icon-thin-0333_printer"></i>  <?php echo getPhrase('print');?></button></a>
                                            </div>
    					                </div>
  					                </div>
				                </div>	
