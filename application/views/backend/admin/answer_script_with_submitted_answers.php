...
?>
    <div style="text-align: center;">
        <h3><?php echo getPhrase('results');?>: <b><?php echo $online_exam_info->title;?></b></h3><br>
    </div>
    ...
    <element class="col-sm-6 col-aligncenter">
        <div class="pipeline white lined-danger">            
            <div class="pipeline-header">
                ...
                <span><?php echo getPhrase('mark');?>: <?php echo $mark;?></span>
            </div>
            <?php 
            ...
            <?php
                if ($row['submitted_answer'] != "" || $row['submitted_answer'] != null) 
                {
                    ...
                    $r = getPhrase('no_reply');
                }
            ?>
            <i><strong>[<?php echo getPhrase('answer');?> - <?php echo rtrim(trim($r), ',');?>]</strong></i>
            <br>
            <?php
              ...
              $r = getPhrase('none_of_them.');
            }
          ?>
            <i><strong>[<?php echo getPhrase('correct_answer');?> - <?php echo rtrim(trim($r), ',');?>]</strong></i>
          ...
          ?>
            ...
            ?>
            <i><strong>[<?php echo getPhrase('answer');?> - <img width="30px" onclick="showAjaxModal('<?php echo base_url();?>modal/popup/modal_question_image/<?php echo rtrim(trim($r), ',');?>');" src="<?php echo base_url();?>public/uploads/online_exam/<?php echo rtrim(trim($r), ',');?>">]</strong></i>
            <br>
            <?php
              if ($row['correct_answers'] != "" || $row['correct_answers'] != null) {
              $correct_options = json_decode($row['correct_answers']);
          ...
          ?>
            <i><strong>[<?php echo getPhrase('correct_answer');?> - <img width="30px" onclick="showAjaxModal('<?php echo base_url();?>modal/popup/modal_question_image/<?php echo rtrim(trim($r), ',');?>');" src="<?php echo base_url();?>public/uploads/online_exam/<?php echo rtrim(trim($r), ',');?>">]</strong></i>
          <?php elseif($question_type == 'fill_in_the_blanks'):
            if ($row['submitted_answer'] != "" || $row['submitted_answer'] != " ") {
                ...
                $submitted_answer =  getPhrase('no_reply');
            }
            $suitable_words   = implode(',', json_decode($row['correct_answers']));
          ?>
            <i><strong>[<?php echo getPhrase('answer');?> - <?php echo $submitted_answer;?>]</strong></i><br/>
            <i><strong>[<?php echo getPhrase('correct_answer');?> - <?php echo $suitable_words;?>]</strong></i>
          <?php elseif($question_type == 'true_false'):
              if ($row['submitted_answer'] != "") {
                  $submitted_answer = getPhrase($row['submitted_answer']);
              }
              else{
                  $submitted_answer = getPhrase('no_reply');
              }
              ?>
              <i><strong>[<?php echo getPhrase('answer');?> - <?php echo getPhrase($submitted_answer);?>]</strong></i><br>
              <i><strong>[<?php echo getPhrase('correct_answer');?> - <?php echo getPhrase($row['correct_answers']);?>]</strong></i>
          <?php endif; ?>
        </div>
    </element>
<?php endforeach;?>