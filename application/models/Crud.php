...
class Crud extends School 
{
    ...
    function createBook()
    {
        ...
        if($this->input->post('type')  == 'virtual')
        {
            ...
            {
                $this->session->set_flashdata('error_message' , getPhrase('extension_not_allowed'));
            }
        }else
        {
            $data['name']        = $this->input->post('name');
    ...
    function updateSettings()
    {
        ...
        $data['description'] = strip_tags($this->input->post('running_semester'));
        $this->db->where('type' , 'running_semester');
        $this->db->update('settings' , $data);
    }
    ...
    function send_news_notify()
    {
        ...
        $notify['notify'] = getPhrase('new_notice_info');
    ...
    {
        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $notify['notify'] = getPhrase('new_poll_notify');
        $parents = $this->db->get('parent')->result_array();
        $students = $this->db->get_where('enroll', array('year' => $year))->result_array();
        $teachers = $this->db->get('teacher')->result_array();
        $accountant = $this->db->get('accountant')->result_array();
    ...
    {
        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $notify['notify'] = getPhrase('new_event_notify');
        $parents = $this->db->get('parent')->result_array();
        $students = $this->db->get_where('enroll', array('year' => $year))->result_array();
        $teachers = $this->db->get('teacher')->result_array();
        $accountant = $this->db->get('accountant')->result_array();
    ...
    function add_multiple_choice_question_to_online_exam($online_exam_id){
        if (sizeof($this->input->post('options')) != $this->input->post('number_of_options')) {
            $this->session->set_flashdata('error_message' , getPhrase('no_options_can_be_blank'));
            return;
        }
    ...
    function createTeacherReport()
    {
        ...
        $this->db->insert('student_report', $data);
    ...
    function send_reply_group_message($message_thread_code) 
    {
       ...
       if(!file_exists('public/uploads/group_messaging_attached_file/')) 
        ...
        if ($_FILES['attached_file_on_messaging']['name'] != "") 
        {
            ...
            {
                $this->session->set_flashdata('error_message' , getPhrase('2MB_allowed'));
    ...
    function get_years($status) {
        $query = $this->db->get_where('years', array('status' => $status));
        return $query->result_array();
    }

    function get_periods($status) {
        $query = $this->db->get_where('semesters', array('status' => $status));
        return $query->result_array();
    }
    ...
    function get_gpa($mark_obtained) 
    {
    ...
    function panelDate()
    {
        $days = array(getPhrase("Monday"),getPhrase("Tuesday"),getPhrase("Wednesday"),getPhrase("Thursday"),getPhrase("Friday"),getPhrase("Saturday"),getPhrase("Sunday"));
        return $days;
    }
    ...
    function delete_thread_messages($message_thread_code) {
        $this->db->where('message_thread_code', $message_thread_code);
        $this->db->update('message_thread', array('status' => 0));
    }
    ...
    function permission_request()
    {
        ...
        $data['year']         = $this->runningYear;
        $data['semester_id']  = $this->runningSemester;
        $this->db->insert('teacher_request', $data);
    ...
    function updateAcademicSettings()
    {
        ...
        $data['description']  = html_escape($this->input->post('terms'));
        ...
        $data['description']  = $this->input->post('running_semester');
        $this->db->where('type' , 'running_semester');
        $this->db->update('settings' , $data);
    }
    ...
    function attendanceUpdate($class_id, $section_id,$subject_id, $timestamp)
    {
        ...
        foreach($attendance_of_students as $row) 
        {
            ...
            if ($attendance_status == 2) 
            {
                ...
                $message        = getPhrase('your_child') . ' ' . $student_name . getPhrase('is_absent_today');
                if($notify == 1)
                {
                    if ($sms_status == 'msg91') 
                    {
